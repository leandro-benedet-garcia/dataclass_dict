...
from dataclasses import _FIELDS, _process_class, dataclass, field
...
class DataclassDict(MutableMapping, KeysView):
    ...
    def __new__(cls, *_, **kwargs: Dict[str, Any]) -> "DataclassDict":
        ...
        for cur_key, cur_value in kwargs.items():
            assert valid_variable(cur_key), f"the variable {cur_key} has a invalid name"
        ...
        # Even if the class has an annotated attribute,
        # It should not appear in the dataclass at all if it starts with `_`
